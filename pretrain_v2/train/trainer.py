...
from ..data import generate_variable_padded_batch
from ..model import NanoTabPFNPUModel
from ..simplified_prior import SimplifiedPriorConfig, sample_curriculum_config
    ...
    for idx in range(config.eval_batches):
        ...
        def _cfg_sampler(local_rng: np.random.Generator) -> SimplifiedPriorConfig:
            return sample_curriculum_config(
                ...
                pu_schedule=config.data.pu_schedule,
...
def pretrain_nano_tabpfn_pu(
    ...
    for step in range(start_step, config.total_steps):
        ...
        rec = {
            ...
            "realized_unlabeled_to_positive_ratio": float(batch["realized_unlabeled_to_positive_ratio"].mean().item()),
            "requested_test_class1_ratio": float(batch["requested_test_class1_ratio"].mean().item()),
            "realized_train_class1_ratio": float(batch["realized_train_class1_ratio"].mean().item()),
